-- Flat Ad & Royalties Fee — remove per-invoice persistence.
--
-- This migration:
--   1. Snapshots the pre-migration invoices rows into a backup table (F2).
--   2. Adds grand_total_original audit column and preserves the original total (F3).
--   3. Rewrites grand_total to ROUND(subtotal + tax_amount, 2) for all rows (F8, idempotent).
--   4. Drops the invoices.ad_royalties_fee column.
--   5. Recreates fulfill_order_with_invoice so it no longer reads the fee or writes it
--      to the invoices row. SECURITY DEFINER SET search_path = public, pg_temp is preserved (F9).
--
-- The financial_settings.ad_royalties_fee key is intentionally NOT dropped — the settings
-- form and both selection summaries still read it.
...
ALTER TABLE invoices DROP COLUMN IF EXISTS ad_royalties_fee;
...
BEGIN
  ...
  -- Calculate tax on subtotal only; grand_total no longer includes ad fee.
